...
export function autoAccount(records){
 ...
 for(const r of rows){
  ...
  if(!r.spamDismissed&&r.asset!=='native'&&(r.sourceSpam||(!knownAssets.has(assetKey(r))&&r.direction==='out'&&lookalike))){
   r.spam=true;r.spamReason=r.sourceSpam?'数据源标记为可疑代币 / 垃圾记录':'合约未匹配返佣资产，且收款地址仿似已知被邀请地址';r.needsReview=true;continue;
  }
  ...
  }else{r.kind='pending';r.needsReview=true;r.reviewReason=r.direction==='out'?(r.chain!=='solana'&&canonical(r.chain,r.txSender)!==own?'尚未证实由本钱包发起；日志发送地址不代表付款授权':'地址、网络或合约尚未匹配返佣'):'未识别到可归属的返佣凭证';}
 }
 return rows;
}
