...
export function isDirectTokenTransfer(tx,row){
 ...
 return /^0xa9059cbb0{24}[0-9a-f]{40}[0-9a-f]{64}$/i.test(input)&&canonical(row.chain,tx.to?.hash||tx.to)===row.asset&&canonical(row.chain,tx.from?.hash||tx.from)===row.from&&'0x'+input.slice(34,74).toLowerCase()===row.to&&BigInt('0x'+input.slice(74)).toString()===row.raw;
}
...
export function autoAccount(records){
 const rows=records.map(r=>{const copy={...r};delete copy.spam;delete copy.spamReason;if(copy.whitelistKind){copy.kind=copy.whitelistKind;delete copy.whitelistKind}if(copy.automatic&&!copy.reviewed){copy.kind='pending';delete copy.automatic}if(copy.autoExcluded&&!copy.reviewed&&!copy.supersededBy){copy.kind='pending';delete copy.autoExcluded}if(!assetAllowed(copy.chain,copy.asset)){copy.whitelistKind=copy.kind;copy.kind='pending';copy.spam=true;copy.spamReason=revokedAsset(copy.chain,copy.asset)?'已撤销信任的假 USDbC 合约；不属于官方桥接 USDbC':'合约 / Mint 不在当前网络白名单';copy.needsReview=true}return copy});const receiptIds=new Set(records.flatMap(r=>r.supersededBy||[]));
 for(const r of rows){
  if(r.chain==='solana'&&r.direction==='in'&&r.asset==='native'&&BigInt(r.raw)>0n&&BigInt(r.raw)<=1n){r.kind='pending';r.spam=true;r.spamReason='1 lamport SOL 微量转入（dusting）';r.needsReview=true;continue}
  if(r.chain==='solana'&&r.solanaSelfSwap&&!r.spam&&!r.reviewed){r.kind='ignore';r.trader='';r.autoExcluded=true;r.exclusionReason='本钱包自己的 swap，非返佣 / 返还'}
 }
 ...
 for(const r of rows){
  ...
  else if(r.chain==='solana'&&r.direction==='in'&&r.solanaCommission&&r.attributionVerified&&r.trader!==SOL){r.kind='commission';r.automatic=true;r.evidence='自动归属：OKX SwapV3 返佣账户实际到账，与签名用户匹配'}
 ...
 for(const r of rows)if(r.kind==='pending'&&!r.reviewed&&!r.feeEvent&&!r.supersededBy&&r.chain==='8453'&&r.direction==='in'&&r.directTokenTransfer&&fixedUsdPrice(r.chain,r.asset)&&Number.isInteger(r.decimals)&&r.decimals>=0&&r.decimals<=36&&BigInt(r.raw)>0n&&BigInt(r.raw)*1000n<10n**BigInt(r.decimals)){r.spam=true;r.spamReason='Base 稳定币直接微额转入 < 0.001 USD（dusting）';r.needsReview=true}
 return rows;
}
